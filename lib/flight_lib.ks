...
FUNCTION AUTODRIVE_TO_WAYPOINT {
    ...
    IF CURR_WAYPOINT="NONE" {
        RETURN.
    }
    CLEARSCREEN.
    SET TVAL TO 0.
    SET BEARING TO COMPASS_FOR(SHIP, CURR_WAYPOINT).
    LOCK THROTTLE TO TVAL.
    LOCK STEERING TO HEADING(BEARING, 0).
    UNTIL CURR_WAYPOINT:POSITION:MAG < 300 {
        SET BEARING TO COMPASS_FOR(SHIP, CURR_WAYPOINT).
        SET TVAL TO MAX(0, (MAX_SPEED - SHIP:GROUNDSPEED)/MAX_SPEED).
        PRINT "BEARING: " + ROUND(BEARING) + " DEGREES    " AT (5, 3).
        PRINT "DISTANCE: " + ROUND(CURR_WAYPOINT:POSITION:MAG()) + " METERS    " AT (5, 4).
...
FUNCTION AUTOPILOT_TO_WAYPOINT {
    PARAMETER HEIGHT.
    IF SHIP:ALTITUDE > 14000 {
        // GLIDE DOWN FIRST AND REGAIN CONTROL.
        GLIDE_DOWN(14000).
    }
    SET CURR_WAYPOINT TO GET_SELECTED_WAYPOINT().
    IF CURR_WAYPOINT="NONE" {
    ...
    SAS OFF.
    UNLOCK STEERING.
    LOCAL BEARING TO COMPASS_FOR(SHIP, CURR_WAYPOINT).
    LOCAL PITCHPID TO PIDLOOP(0.01, 0.0001, 0.005, -1, 1).
    LOCAL ROLLPID TO PIDLOOP(0.002, 0.0001, 0.004, -1, 1).
    LOCAL TVAL TO 1.
    LOCK THROTTLE TO TVAL.
    LOCK MAP_DIST TO VXCL(SHIP:UP:VECTOR, CURR_WAYPOINT:POSITION):MAG.

    UNTIL MAP_DIST < 6500 {
        LOCAL TVAL_SCALAR TO MIN(1, 900/(SHIP:GROUNDSPEED+1)).
        IF MAP_DIST < 40000 {
            SET WARP TO 0.
            SET HEIGHT TO CURR_WAYPOINT:ALTITUDE.
        }
        LOCAL COMPASS TO COMPASS_FOR().
        LOCAL DIRECTION TO COMPASS_FOR(SHIP, CURR_WAYPOINT).
        LOCAL PITCH TO PITCH_FOR().
        LOCAL ROLL TO ROLL_FOR().
        LOCAL PITCH_AIM TO MAX(-10, MIN((HEIGHT - SHIP:ALTITUDE)/100, 25)).
...
FUNCTION LANDING {
    ...
    UNTIL ALT:RADAR < 2 {
        ...
        PRINT "PITCH: " + ROUND(PITCH) + " DEGREES     " AT (5, 2).
        PRINT "ROLL: " + ROUND(ROLL) + " DEGREES     " AT (5, 3).
        PRINT "ALT: " + ROUND(ALT:RADAR) + " METERS     " AT (5, 4).
        PRINT "SPEED: " + ROUND(SHIP:GROUNDSPEED*3.6) + " KPH    " AT (5, 5).
    }
    CLEARSCREEN.
    UNLOCK THROTTLE.
...
FUNCTION GLIDE_DOWN {
    PARAMETER END_ALTITUDE TO 10000.
    WAIT 1.
    UNLOCK STEERING.
    SAS OFF.
    CLEARSCREEN.
    LOCAL PLOOP TO PIDLOOP(0.05, 0.001, 0.05, -1, 1).
    LOCAL RLOOP TO PIDLOOP(0.002, 0.001, 0.01, -1, 1).
    LOCK THROTTLE TO 0.
    UNTIL SHIP:ALTITUDE < END_ALTITUDE {
        SET PLOOP:SETPOINT TO -8.
        LOCAL PITCH TO PITCH_FOR().
        LOCAL ROLL TO ROLL_FOR().
        SET SHIP:CONTROL:PITCH TO PLOOP:UPDATE(TIME:SECONDS, PITCH).
        SET SHIP:CONTROL:ROLL TO RLOOP:UPDATE(TIME:SECONDS, ROLL).
