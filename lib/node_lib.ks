...
FUNCTION NODE_EXEC {
    ...
    CLEARSCREEN.
    SAS OFF.
    LOCAL CURR_NODE TO NEXTNODE.
    LOCAL DURATION TO CURR_NODE:DELTAV:MAG/(SHIP:MAXTHRUST/SHIP:MASS).
    WARP_TO_TIME(CURR_NODE:ETA - DURATION/2 - 20).
    LOCK STEERING TO CURR_NODE:DELTAV.
    WAIT CURR_NODE:ETA - DURATION/2.

    LOCAL TVAL TO 0.
    LOCK THROTTLE TO TVAL.
    LOCAL OG_NODE_HEADING TO CURR_NODE:DELTAV.
    UNTIL FALSE {
        IF STAGE:LIQUIDFUEL < 1 AND ALLOW_STAGE {
            STAGE. WAIT 0.
        }
        LOCAL MAX_ACC TO SHIP:MAXTHRUST/SHIP:MASS.
        SET TVAL TO MIN(CURR_NODE:DELTAV:MAG/MAX_ACC, 1).
        IF VDOT(OG_NODE_HEADING, CURR_NODE:DELTAV) < 0 OR CURR_NODE:DELTAV:MAG < 0.1 {
            BREAK.
        }
        ...
        PRINT "DV LEFT: " + ROUND(CURR_NODE:DELTAV:MAG) + " M/S     " AT (5,5).
    }
    SET TVAL TO 0.
    WAIT 0.
    UNLOCK STEERING.
